...
for type in build_type:
  logger.info(f"Starting build {type}")
  subprocess.run(['conan', 'build', '.', '-s', f'build_type={type}', '-c', 'tools.build:skip_test=False', '-pr:a=./ci/src/conan-profile/profile-release-gcc-13-x86_64', '--build="*"'], check=True)
